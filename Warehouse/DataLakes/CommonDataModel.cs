...
namespace Bygdrift.Warehouse.DataLakes
{
    class CommonDataModel
    {
        ...
        {
            dataLake = new DataLake(connectionString, container, module);
            ...
            if (Refines != null)
            {
                ...
                foreach (var refine in Refines)
                    if (refine.CsvAddToCommonDataModel)
                        CreateEntity(ref entities, refine.TableName, refine.Csv, refine.CsvBasePath);
        ...
        private void Validate()
        {
            foreach (var item in Refines)
            {
                ...
                foreach (var header in item.Csv.Headers)
                    if (item.Csv.Headers.Count(o => o.Value == header.Value) > 1)
        ...
        internal void CreateEntity(ref JArray entities, string tableName, Csv csv, string path)
        {
