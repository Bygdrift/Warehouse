...
namespace Bygdrift.Warehouse.DataLakes
{
    ...
    {
        public static IEnumerable<KeyValuePair<DateTime, Csv>> GetFilesFromDataLake(this DataLake dataLake, string tableName, string basePath, DateTime from, DateTime to)
        {
                        ...
                        if (int.TryParse(Path.GetFileName(monthFolder.Name), out int month))
                            foreach (var dayFolder in fileSystem.GetPaths(monthFolder.Name).Where(o => o.IsDirectory == true))
                                if (int.TryParse(Path.GetFileName(dayFolder.Name), out int day))
                                {
                                    ...
                                    var res = GetCsv(dataLake, fileSystem, dayFolder, tableName, from, to, date);
                                    if (res != null)
                                    ...
                                    foreach (var hourFolder in fileSystem.GetPaths(dayFolder.Name).Where(o => o.IsDirectory == true))
                                        if (int.TryParse(Path.GetFileName(hourFolder.Name), out int hour))
                                        {
                                            ...
                                            res = GetCsv(dataLake, fileSystem, hourFolder, tableName, from, to, date);
                                            if (res != null)
                                                yield return new KeyValuePair<DateTime, Csv>(date, res);
                                        }
                                }
        ...
        public static Csv GetCsv(this DataLake dataLake, DataLakeFileSystemClient fileSystem, PathItem pathItem, string tableName, DateTime from, DateTime to, DateTime date)
        {
